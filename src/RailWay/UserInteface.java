...
public class UserInteface {
    ...
    private void runCommandDepot(Command command) throws IncorrectInputException{
        switch(command.getCommand()) {
            case "cre": //create engine <engineType> <class> <name> <length> <couplingFront> <couplingBack> + cre
                depot.createEngine(command.getValue()[0], command.getValue()[1], command.getValue()[2], Integer.parseInt(command.getValue()[3]),
            ...
            case "crc": //create coach <coachType> <length> <couplingFront> <couplingBack> + crc
                depot.createCoach(command.getValue()[0], Integer.parseInt(command.getValue()[1]), command.getValue()[2].equals("true"), command.getValue()[3].equals("true"));
                break;
            ...
            case "cts": //create train-set <class> <name> <length> <couplingFront> <couplingBack> + cts
                depot.createTrainSet(command.getValue()[0], command.getValue()[1], Integer.parseInt(command.getValue()[2]), command.getValue()[3].equals("true"), command.getValue()[4].equals("true"));
                break;
    ...
    private void runCommandState(Command command) throws IncorrectInputException{
        switch(command.getCommand()) {
            ...
            case "put": //put train <trainID> at <point> in direction <x>,<y> + put
                state.putTrain(depot.getTrain(Integer.parseInt(command.getValue()[0])), extractPoint(command.getValue()[1]), extractPoint(command.getValue()[2]));
                break;
    ...
    //add track <startpoint> -> <endpoint> + atk
    //add switch <startpoint> -> <endpoint1>,<endpoint2> + asw
    //delete track <trackID> + dtk
    //list tracks + ltr
    //set switch <trackID> position <point> + set
    //create engine <engineType> <class> <name> <length> <couplingFront> <couplingBack> + cre
    //list engines + lte
    //create coach <coachType> <length> <couplingFront> <couplingBack> + crc
    //list coaches + ltc
    //create train-set <class> <name> <length> <couplingFront> <couplingBack> + cts
    //list train-sets + lts
    //delete rolling stock <id> + dls
    //delete train <ID> + dlt


    //add train <trainID> <rollingStockID> + atn
    //list trains + lit
    //show train <trainID> + swt
    //put train <trainID> at <point> in direction <x>,<y> + put
    //step <speed> + stp
    //exit + ext
    private Command readCoomand(String userInput) throws IncorrectInputException{
    ...
    private Command readPut(String[] str) throws IncorrectInputException{
        if(str.length == 8 && str[1].equals("train") && str[2].matches("\\d+") && str[3].equals("at") && str[4].matches(POINT_REGEX) &&
    ...
    //TODO here Error, Incorrect Input
    private Command readSet(String[] str) throws IncorrectInputException{
        if(str.length == 5 && str[1].equals("switch") && str[2].matches("^\\d+") && str[3].equals("position") && str[4].matches(POINT_REGEX)) {
    ...
    private Command readCreate(String[] str) throws IncorrectInputException{
        if (str.length < 2)
            ...
            else if(str[1].equals("coach") && str.length == 6 && str[2].matches("passenger|freight|special") && str[3].matches("\\d+")
                    && str[4].matches("true|false") && str[5].matches("true|false"))
                return new Command(new String[]{str[2], str[3], str[4], str[5]}, "crc");
            else if(str[1].equals("train-set") && str.length == 7 && str[2].matches(SCLASS_REGEX) && str[3].matches("\\w+") &&
